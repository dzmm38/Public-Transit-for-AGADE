package PublicTransportRouting.PT_Graph;

import PublicTransportRouting.supClass.TransitionConfig;
...
public class PT_Graph {
    ...
    public static void createGraph(String OsmFile, String GtfsFile){
        ...
        TransitionConfig transitionConfig = new TransitionConfig(config.getString("graph.flag_encoders",""),config.getString("datareader.file",""),
                                                              config.getString("graph.location",""),config.getString("gtfs.file",""));

        //Creating an Object Mapper and YAML Factory to save the transitionConfig as a file in the associated graph folder
        ObjectMapper om = new ObjectMapper(new YAMLFactory());
        try {
            om.writeValue(new File(graphLocation+"\\"+"config_pt.yml"), transitionConfig);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public PtRouteResource loadGraph(String graphFolderName){
        ...
        config = ladeConfig(graphFolderName);           //Load the config of an existing graph to load the graph again
     ...
     */
    private GraphHopperConfig ladeConfig(String graphFolderName){
        ...
        ObjectMapper om = new ObjectMapper(new YAMLFactory());
        try {
            config = om.readValue(new File(ConfigPath), TransitionConfig.class).switchConfigs();        //switchConfigs to create a GraphopperConfig file
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
