...
public class RouteLoader {
    ...
    public void loadRouteFromFile(String FilePath) throws IOException {
        ...
        if (FilePath.contains(".yml")) {
            ...
            routeArray = om.readValue(new File(FilePath), Route[].class);    //reading the YAML file in an array, every entry represents a Route
        } else if (FilePath.contains(".json")) {
            om = new ObjectMapper(new JsonFactory());
            routeArray = om.readValue(new File(FilePath), Route[].class);    //reading the JSON file in an array, every entry represents a Route
        } else {
    ...
    //--------------------------------------- Getter & Setter ---------------------------------------//
    public Route[] getRouteArray() {
        return routeArray;
    }

    public void setRouteArray(Route[] routeArray) {
